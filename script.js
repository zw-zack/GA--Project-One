...
function gameStart() {
	...
	function makeDinos(){
		var myDino = new enemyTypeTwo(70, 70, "dino.png", 1100, 650*Math.random(), "image");
		dinos.push(myDino);
	}
	...
	function makeGhosts(){
		for(var i = 0; i<3 ; i++){
			var myGhost = new enemyTypeThree(60, 60, "ghost.png", 30, 650*Math.random(), "image");
			ghosts.push(myGhost);
		}
	}
	...
	function makeCake(){
		var myCake = new friendliesTypeOne(20, 20, "cake.jpg", Math.random()*1100 , Math.random()* 550, "image");
		cakes.push(myCake);
	}

	var cakeInterval = setInterval(makeCake, 10000);
	myGameArea.start();
}
...
//dinos
	function enemyTypeTwo(width, height, color, x, y, type) {
		this.type = type;
		if (type == "image") {
			this.image = new Image();
			this.image.src = color;
...
//ghosts
		function enemyTypeThree(width, height, color, x, y, type) {
		this.type = type;
		if (type == "image") {
			this.image = new Image();
			this.image.src = color;
...
//cake
	function friendliesTypeOne(width, height, color, x, y, type) {
		this.type = type;
		if (type == "image") {
			this.image = new Image();
			this.image.src = color;
	...
	function updateGameArea() {
		...
		for( var i = 0; i < cakes.length; i++ ) {
			console.log("cakes")
			cakes[i] = { x: 0, y: 0, status: 1 };

			console.log(cakes.status);
			//cakes[i].update();
			//cakes[i].newPos();
		}
	}
